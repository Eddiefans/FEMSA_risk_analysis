...
Si bien, la siguiente ecuación se utiliza para estimar los cambios en el precio:

$$ln(S_t) = ln(S_0)+(\mu-\frac{1}{2}\sigma^2)dt+\sigma dW$$

Para trabajar directamente con el precio y realizar sus estimaciones se utiliza: 
...
```{r}
cat("(",mean(St)+z*sd(St),",",mean(St)-z*sd(St),")")
```
